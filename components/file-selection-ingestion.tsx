...
export default function FileSelectionForIngestion({ files, setFiles }: FileSelectionForIngestionProps) {
  ...
  const getQualityScore = (file: FileData) => {
    ...
    return (
      (file.qualityMetrics.parseAccuracy + file.qualityMetrics.completeness + file.qualityMetrics.complexity) / 3
    )
  }
  ...
  const structuredCount = processedFiles.filter((f) => f.classification === "Structured").length
  const semiStructuredCount = processedFiles.filter((f) => f.classification === "Semi-Structured").length
  const unstructuredCount = processedFiles.filter((f) => f.classification === "Unstructured").length
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-blue-600">{structuredCount}</div>
            <div className="text-sm text-gray-600">Structured Files</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-purple-600">{semiStructuredCount}</div>
            <div className="text-sm text-gray-600">Semi-Structured Files</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-gray-600">{unstructuredCount}</div>
              ...
              {filteredAndSortedFiles.map((file) => {
                ...
                return (
                  <TableRow key={file.id}>
                    ...
                    <TableCell>{file.classification && getClassificationBadge(file.classification)}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                    ...
                    <TableCell>
                      <Badge variant="outline">{targetDb}</Badge>
                    </TableCell>
                  </TableRow>
                )
              })}
